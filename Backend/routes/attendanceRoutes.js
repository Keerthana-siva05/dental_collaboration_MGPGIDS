...
import AverageAttendance from "../models/averageAttendanceModel.js";
    ...
    try {
      ...
      if (!/^\d{4}-\d{4}$/.test(batch)) {
        return res.status(400).json({ 
          success: false,
          message: "Batch must be in YYYY-YYYY format (e.g., 2021-2025)" 
        });
      }
      ...
      if (endYear - startYear !== 4) {
        return res.status(400).json({ 
          success: false,
          message: "Batch must cover a 4-year period (e.g., 2021-2025)" 
        });
      }
      ...
      if (!students || students.length === 0) {
        return res.status(404).json({ 
          success: false,
          message: "No students found for the selected course and batch" 
        });
      }
  
      // Transform student data to match attendance format
      const formattedStudents = students.map(student => ({
        regNumber: student.registerNo, // Map registerNo to regNumber
        name: student.name,
